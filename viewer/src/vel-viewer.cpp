...
static unsigned int dT = 5;	// 5ms
static int keyframeSkip = 5;		// キーフレーム間隔
...
int main( int aArgc, char *aArgv[ ] )
{
	...
	try {
		...
		GraphDrawerVel drawer;
		drawer.setParameters( flag_save );
		drawer.initGnuplot( );
...
//			printf("(%d:%d)\n", update_id[ INDEX_LOCAL ],getTID_top( localizer.getSSMId( ) ) );
			...
			if( update[ INDEX_LOCAL ] ){
				...
				drawer.setVel( &localizer.data, localizer.time );
...
static void printShortHelp( const char *programName )
{
	...
	printf( "\t-s | --save         SAVE   : Save localizer data to log_localizer.dat (default=%d)\n", flag_save );
}
static bool setOption(	int aArgc, char *aArgv[] )
{
	int opt, optIndex = 0;
