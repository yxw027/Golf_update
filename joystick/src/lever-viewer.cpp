...
int main( int aArgc, char *aArgv[ ] )
{
	...
	SSMApi<OMcntl> lever( OM_CNTL_SNAME, 0 );
		OMotor = &lever;
	...
	try {
		...
		GraphDrawerLever gdrawer;		// gnuplotによる描画
		gdrawer.setParameters( _LEVER, flag_save );
...
#define INDEX_LEVER			0
	
		while( !gShutOff ){
			update[ INDEX_LEVER ] = false;
			// get latest data for INDEX_LEVER
			if( update_id[ INDEX_LEVER ] < getTID_top( lever.getSSMId( ) ) ){
				lever.readLast(  );
				update[ INDEX_LEVER ] = true; // 最新情報を読み込む
				update_id[ INDEX_LEVER ] = lever.timeId;
			} else {
				update[ INDEX_LEVER ] = false;
			}
			
			if( update[ INDEX_LEVER ] ){
				
				gdrawer.setLeverInfo( &lever.data, lever.time );
				gdrawer.drawGraph( );
